...
T['semaphore'] = MiniTest.new_set()

T['semaphore']['runs'] = function()
  local ret = {}
  async
    .run(function()
...
T['semaphore']['new defaults to 1 permit'] = function()
  local sem = async.semaphore()
  eq(sem:available(), 1)
end

T['semaphore']['new creates with specified permits'] = function()
...
T['semaphore']['acquire decrements available count'] = function()
  local sem = async.semaphore(3)
  async
    .run(function()
      sem:acquire()
...
T['semaphore']['release increments available count'] = function()
...
T['semaphore']['release errors when exceeding max permits'] = function()
...
T['semaphore']['second acquire blocks when no permits available'] = function()
  local sem = async.semaphore(1)
  local order = {}
...
T['semaphore']['n permits allow n concurrent acquires'] = function()
  local sem = async.semaphore(3)
  local inside = 0
  local max_inside = 0
  ...
  -- At most 3 tasks should have been inside concurrently
  assert(max_inside <= 3, 'Expected max_inside <= 3, got: ' .. tostring(max_inside))
  -- At least 3 should have been concurrent (we have 5 tasks with 3 permits)
  assert(max_inside >= 2, 'Expected max_inside >= 2, got: ' .. tostring(max_inside))
end

T['semaphore']['waiters are woken in FIFO order'] = function()
  local sem = async.semaphore(1)
  local order = {}
...
T['semaphore']['available is 0 when fully acquired'] = function()
  local sem = async.semaphore(2)
  async
    .run(function()
      sem:acquire()
...
T['semaphore']['mutual exclusion: critical section runs serially'] = function()
  local sem = async.semaphore(1)
  local counter = 0
  local races = 0
    ...
    .run(function()
      ...
      for i = 1, 10 do
        tasks[i] = async.run(function()
          ...
          sleep(1)
          -- If another task entered concurrently, counter would have changed
